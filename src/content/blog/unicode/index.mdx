...
UTF-8 是一种可变长度编码。码位可能被编码为一个到四个字节的序列。
...
<Code lang="js" code={`var bytes = "Аналитика".getBytes("UTF-8");
...
- <code class="emoji">🚵🏻‍♀️</code> is `U+1F6B5` + `U+1F3FB` + `U+200D` + `U+2640` +
  `U+FE0F`
...
<Loud>
  An Extended Grapheme Cluster is a sequence of one or more Unicode code points
  that must be treated as a single, unbreakable character.
</Loud>
...
<b>但是！</b>
如果你问任何正常的人，一个不被计算机内部所拖累的人，他们会给你一个直接的答案：1。`🤦🏼‍♂️`字符串的长度是
1。
...
这就是扩展字位簇的全部内容：人们认为是单个字符。在这种情况下，`🤦🏼‍♂️`无疑是一个单独的字符。
...
1. 外部，面向人类的 API。UI 中的字符计数。获取前 10 个字符以生成预览。在文本中搜索。像 `.count` 或 `.substring` 这样的方法。Swift 给你<em>一个视图</em>，假装字符串是一个字位簇序列。这个视图的行为就像任何人所期望的那样：它为 `"🤦🏼‍♂️".count` 给出 1。
...
但这正是你应该做的！使用一个合适的 Unicode 库！是的，对于像 `strlen{:js}` 或 `indexOf{:js}` 或 `substring{:js}` 这样的基本功能！
...
简短的答案：它不知道。不幸的是，Unicode 不是一个完美的系统，它有很多缺点。其中之一是将相同的码位分配给应该看起来不同的字形，比如西里尔小写字母 K 和保加利亚小写字母 K（都是 `U+043A`）。
...
<Loud lang='js' code='var en_US = Locale.of("en", "US");
var tr = Locale.of("tr");
...
- 对于所有 Unicode 相关的东西，甚至是像 `strlen{:js}`、`indexOf{:js}` 和 `substring{:js}` 这样的无聊的东西，都要使用 Unicode 库。
