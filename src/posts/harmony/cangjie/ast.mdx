...
{/* 本次实验报告深入探讨了仓颉语言的特性实践与编译原理知识的应用，并初步探索了现代 IDE 功能的模拟与 LLM 的集成。在仓颉语言特性方面，核心在于对抽象语法树（AST）的操作。在编译原理知识应用方面，虽然直接使用了标准库的解析器，但通过实践，更深刻地理解了从词法分析、语法分析到生成抽象语法树（AST）的过程，并体会到 AST 在代码分析、格式化、重构和优化中的核心地位。本次作业实现的各项功能，如代码转换和分析，本质上都是静态分析的具体应用。通过实现这些简化的 IDE 功能，例如代码结构查看和简单的转换，对现代 IDE 如何辅助开发者提升代码质量和开发效率有了更直观的感受。最后，首次尝试将大语言模型应用于代码辅助工具（文档生成），初步体验了 AI 在软件开发领域的巨大潜力。 */}
