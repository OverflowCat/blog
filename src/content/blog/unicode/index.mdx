...
# 2023 年每个软件开发者都必须知道的关于 Unicode 的最基本的知识（仍然没有任何借口！）
...
实际上，Unicode 是一个将不同字符分配给唯一数字的表格。
...
你书的队。[^touche]
[^touche]: 原文是法语<q lang="fr">Touché</q>，意为「说得好」、「一针见血」。
...
<dfn>代理对</dfn>（<span lang="en">surrogate pair</span>）是用于编码单个 Unicode 码位的两个 UTF-16 单位。例如，`D83D DCA9`（两个 16 位单位）编码了一个码位，`U+1F4A9`。
...
<div class="footer">
  2023 年 10 月 2 日 · 在
  [HackerNews](https://news.ycombinator.com/item?id=37735801) 上讨论 ·
  翻译于 2023 年 10 月 27 日
</div>
