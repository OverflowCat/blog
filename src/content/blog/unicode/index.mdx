...
- <code lang="ko">각</code> (韩语) 是 `U+1100 U+1161 U+11A8`。
...
简短的回答：它不知道。不幸的是，Unicode 不是一个完美的系统，它有很多缺点。其中之一就是是将相同的码位分配给应该看起来不同的字形，比如西里尔小写字母 <span lang="ru">K</span> 和保加利亚语小写字母 <span lang="bg">K</span>（都是 `U+043A`）。
...
[^twitter]: Twitter 错误渲染俄语为保加利亚语——注意 <span lang='bg'>и</span>、<span lang='ru'>й</span>、<span lang='bg'>ь</span>、<span lang='bg'>к</span>、<span lang='bg'>з</span> 等字母的字形。
